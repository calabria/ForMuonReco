...
class TestDetLayer : public EDAnalyzer {
 public:

  TestDetLayer( const ParameterSet& pset);
  ...
  void testGEMLayers(const MuonDetLayerGeometry*, const MagneticField* field);
...
TestDetLayer::TestDetLayer(const ParameterSet& iConfig) 
{
  float theMaxChi2=25.;
  float theNSigma=3.;
...
void TestDetLayer::analyze( const Event& ev,
				       const EventSetup& es ) {
  ...
  cout << "*** allGEMLayers(): " << geo->allGEMLayers().size() << endl;
  for (vector<DetLayer*>::const_iterator dl = geo->allGEMLayers().begin();
       dl != geo->allGEMLayers().end(); ++dl) {
    cout << "  " << (int) (dl-geo->allGEMLayers().begin()) << " " << dumpLayer(*dl);
  }
  cout << endl << endl;

  cout << "*** forwardGEMLayers(): " << geo->forwardGEMLayers().size() << endl;
  for (vector<DetLayer*>::const_iterator dl = geo->forwardGEMLayers().begin();
       dl != geo->forwardGEMLayers().end(); ++dl) {
    cout << "  " << (int) (dl-geo->forwardGEMLayers().begin()) << " " << dumpLayer(*dl);
  }
  cout << endl << endl;

  cout << "*** backwardGEMLayers(): " << geo->backwardGEMLayers().size() << endl;
  for (vector<DetLayer*>::const_iterator dl = geo->backwardGEMLayers().begin();
       dl != geo->backwardGEMLayers().end(); ++dl) {
    cout << "  " << (int) (dl-geo->backwardGEMLayers().begin()) << " " << dumpLayer(*dl);
  }
  cout << endl << endl;
  ...
  testGEMLayers(geo.product(),magfield.product());
}


void TestDetLayer::testDTLayers(const MuonDetLayerGeometry* geo,const MagneticField* field) {
...
void TestDetLayer::testCSCLayers(const MuonDetLayerGeometry* geo,const MagneticField* field) {
  ...
  for (vector<DetLayer*>::const_iterator ilay = layers.begin(); ilay!=layers.end(); ++ilay) {
    ...
    vector<DetLayer::DetWithState> compDets = layer->compatibleDets(tsos,prop,*theEstimator);
    std::cout <<"CSC compatible dets "<<compDets.size()<<std::endl;
...
void TestDetLayer::testGEMLayers(const MuonDetLayerGeometry* geo,const MagneticField* field) {
  const vector<DetLayer*>& layers = geo->allGEMLayers();
  std::cout <<"Number of GEM DetLayer "<<layers.size()<<std::endl;
 for (vector<DetLayer*>::const_iterator ilay = layers.begin(); ilay!=layers.end(); ++ilay) {
    const MuRingForwardDoubleLayer* layer = (const MuRingForwardDoubleLayer*) (*ilay);
    ...
    GlobalTrajectoryParameters gtp(gp,gv,charge,field);
    TrajectoryStateOnSurface tsos(gtp, disk);
    cout << "testGEMLayers: at " << tsos.globalPosition()
         << " R=" << tsos.globalPosition().perp()
         << " phi=" << tsos.globalPosition().phi()
         << " Z=" << tsos.globalPosition().z()
         << " p = " << tsos.globalMomentum()
    ...
    vector<DetLayer::DetWithState> compDets = layer->compatibleDets(tsos,prop,*theEstimator);
    std::cout <<" GEM compatible dets "<<compDets.size()<<std::endl;
    if (compDets.size()) {
	cout << "compatibleDets: " << compDets.size() << endl

           << "  final state pos: " << compDets.front().second.globalPosition() << endl
           << "  det         pos: " << compDets.front().first->position()
           << " id: " << GEMDetId(compDets.front().first->geographicalId().rawId()) << endl
           << "  distance " << (tsos.globalPosition()-compDets.front().first->position()).mag()
      ...
      if(layer->isCrack(gp))
      {
         cout << " GEM crack found ";
      }
      else
      {
        cout << " ERROR : no compatible det found in GEM"
          << " at: R=" << gp.perp()
          << " phi= " << gp.phi().degrees()
          << " Z= " << gp.z();
      }
...
string TestDetLayer::dumpLayer(const DetLayer* layer) const {
  stringstream output;
...
DEFINE_FWK_MODULE(TestDetLayer);
