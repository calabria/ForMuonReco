...

{
   ...
   folderDQM_ = iConfig.getUntrackedParameter<std::string>("folderDQM");
...
BatchSubmission::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   int upperLimit = (int)((sizeInput-1)/200)+1;
   ...
   for(int i = 1; i <= upperLimit; i++){ //Loop sui file di input
	...
	int choice = 0;
	std::string name = "./CFGFiles/" + finalState_ + "/" + sample_ + "/dqmCfg_" + sample_ + "_" + contS + "_cfg.py";
	...
	if(fCFG.is_open()){
			...
			//std::cout<<"lineContent "<<lineContent.c_str()<<std::endl;
	       		...
	       		size_t foundInput, foundOutput, foundFP, foundRootFile;
	       		foundInput = lineContent.find("#__inputFile");
	       		foundOutput = lineContent.find("#__outpuFile");
	       		foundFP = lineContent.find("#__folderPath");
	       		foundRootFile = lineContent.find("#__rootFile");
			if(foundInput!=string::npos) choice = 1;
			else if(foundOutput!=string::npos) choice = 2;
			else if(foundFP!=string::npos) choice = 3;
			else if(foundRootFile!=string::npos) choice = 4;
			else choice = 5;

			int maxFor = i*200 - 1;
			int minFor = i*200 - 200;
				...
				switch(choice){
					case 1:
						...
						break;
					case 2:
						lineContent = "	'" + savingPath_ + sample_ +"/edm_" + sample_ + "_" + contS + ".root'";
						//std::cout<<"lineContent "<<lineContent.c_str()<<std::endl;
						outputCFG<<lineContent<<"\n";
						break;
					case 3:
						lineContent = "	'" + folderDQM_ + "'";
						//std::cout<<"lineContent "<<lineContent.c_str()<<std::endl;
						outputCFG<<lineContent<<"\n";
						break;
					case 4:
			...
			if(foundPath!=string::npos) choice = 1;
				...
				switch(choice){
					case 1:
						...
						break;
					case 2:
						lineContent = "cmsRun " + pathCFG_ + "CFGFiles/" + finalState_ + "/" + sample_ + "/" + "dqmCfg_" + sample_ + "_" + contS + "_cfg.py";
						//std::cout<<"lineContent "<<lineContent.c_str()<<std::endl;
						outputCSH<<lineContent<<"\n";
						break;
					case 3:
